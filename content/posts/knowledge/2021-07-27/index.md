...
前几天在看[GIN-VUE-ADMIN](https://github.com/flipped-aurora/gin-vue-admin)代码的时候，偶然看到一个同过反射去做参数校验的方式。
嘿，学到了！
...
为了调用方便，做了一层封装，使用[函数选项模式](http://localhost:1313/posts/knowledge/2021-07-22/)对校验器进行封装，使调用更为方便。
...
1. 首先是`git commit`、`git push`一把梭把项目整到`GitHub`上。
